...
# 创建一些动态（可选）
for event in RunLog.objects.filter(tenant_id=user.tenant_id)[:50]:  # 前50个事件
    # 提取日期部分
...
print('\n数据创建完成，可以开始压测了！')
print('命令: locust -f locustfile_runlog.py -H http://localhost:80 --users 100 --spawn-rate 20 --run-time 5m --headless')
