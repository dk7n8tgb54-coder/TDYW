...
for name in test_folder_names:
    cursor.execute("DELETE FROM tdyw_document_folder_private WHERE name = %s", [name])

# 删除公共空间测试文件夹
cursor.execute("DELETE FROM tdyw_document_folder_public WHERE name = 'public_test_folder'")

# 删除测试文件
test_file_names = ['file_a.docx', 'file_b.docx', 'duplicate_a.docx', 'duplicate_b.docx', 'move_file_a.docx', 'move_file_b.docx']
for name in test_file_names:
    cursor.execute("DELETE FROM tdyw_document_file_private WHERE name = %s", [name])

# 删除公共空间测试文件
cursor.execute("DELETE FROM tdyw_document_file_public WHERE name = 'public_file.docx'")
