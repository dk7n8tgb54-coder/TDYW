...
职责：
1. 确保公共根目录下存在"行业规章"目录（不存在则创建，存在则复用未删除记录）
2. 创建或更新 DocumentSystemFolder(code='industry_rules') 绑定
3. 幂等执行，便于部署和修复
"""
import logging
...
from apps.document.services.system_folder_service import INDUSTRY_RULES_CODE

logger = logging.getLogger(__name__)

INDUSTRY_RULES_FOLDER_NAME = '行业规章'


class Command(BaseCommand):
    help = '初始化文档系统目录绑定（行业规章）'

    def handle(self, *args, **options):
        self.stdout.write('开始初始化文档系统目录绑定...')

        folder = self._ensure_industry_rules_folder()
    ...
    @transaction.atomic
    def _ensure_industry_rules_folder(self):
        """确保公共根目录下存在"行业规章"目录，复用未删除同名目录"""
        # 默认管理器已过滤 is_deleted=False
        existing = (
            DocumentFolderPublic.objects
            .filter(name=INDUSTRY_RULES_FOLDER_NAME, parent__isnull=True)
            .first()
        )
        if existing:
            self.stdout.write(f'复用已有"行业规章"目录: id={existing.id}')
            return existing

        folder = DocumentFolderPublic.objects.create(
            name=INDUSTRY_RULES_FOLDER_NAME,
            parent=None,
            created_by=None,
        )
        self.stdout.write(f'创建"行业规章"目录: id={folder.id}')
        return folder
    ...
    def _ensure_system_folder_binding(self, folder):
        ...
        binding, created = DocumentSystemFolder.objects.update_or_create(
            code=INDUSTRY_RULES_CODE,
            defaults={
                'name': INDUSTRY_RULES_FOLDER_NAME,
                'folder': folder,
                'is_public': True,
                'protected': True,
                'description': '行业规章系统业务根目录，受保护不可删除/重命名/移动',
            },
        )
        if created:
