#!/usr/bin/env python3
"""
递归搜索功能Docker测试脚本 - 修正版

使用方法：
    docker cp run_search_test.py spug:/data/spug/spug_api/
    docker exec spug sh -c "cd /data/spug/spug_api && python3 run_search_test.py"
"""
    ...
    try:
        ...
        try:
            ...
            user_a = User.objects.create(
                ...
                last_ip='',
                type='default',
                is_supper=True,  # 设置为超级管理员，绕过权限检查
                is_active=True
            )
        ...
        response = view.get(request)
        response_data = json.loads(response.content.decode('utf-8'))
        folders = response_data.get('data', {}).get('folders', [])
        files = response_data.get('data', {}).get('files', [])
