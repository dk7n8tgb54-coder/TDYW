...
- Name: document_admin\document.spec.js >> Document Library - Document Admin >> DOC006 - Cleanup E2E test folders
- Location: tests\document_admin\document.spec.js:101:3
...
```ts
  ...
  88  |     const data = await apiGet(result.context, '/api/document/folders/');
  ...
  96  |     const data = await apiGet(result.context, '/api/document/files/');
  ...
  104 | 
  105 |     // Get all folders
> 106 |     const data = await apiGet(result.context, '/api/document/folders/');
      |                  ^ SyntaxError: Unexpected token '<', "<!DOCTYPE "... is not valid JSON
  ...
  110 |         try {
  111 |           await apiDelete(result.context, `/api/document/folders/${folder.id}/`);
  112 |         } catch (e) {
  ...
  119 |     const filesData = await apiGet(result.context, '/api/document/files/');
  ...
  123 |         try {
  124 |           await apiDelete(result.context, `/api/document/files/${file.id}/`);
  125 |         } catch (e) {
