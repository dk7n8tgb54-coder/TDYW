...
class ComTable extends React.Component {
  ...
  handleDelete = (text) => {
    ...
    Modal.confirm({
      title: '删除确认',
      content: `确定要删除【${logInfo}】的地面干扰记录?`,
      onOk: () => {
        return http.delete('/api/interference/bridge/', {params: {id: text.id}})
          .then(() => {
      ...
      <TableCard
        ...
        actions={[
          ...
          <ExportButton
            ...
            filename="地面干扰导入模板.xlsx">下载导入模板</ExportButton>,
          <ExportButton
            auth="interference.interference.view"
            url="/api/interference/bridge/export/"
            params={store.getExportParams()}
            filename="地面干扰信息.xlsx">导出</ExportButton>
        ]}
        pagination={{
        ...
        <Table.Column title="机号" dataIndex="aircraft_no"/>
