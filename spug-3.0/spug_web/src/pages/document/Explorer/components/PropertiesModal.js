...
export default function PropertiesModal({ visible, record, isPublic, onClose }) {
  ...
  // 打开时调用属性 API（文件夹 + 文件都调用）
  useEffect(() => {
    if (visible && record?.id) {
      setLoading(true);
      setProperties(null);
      http.get('/api/document/folder/properties/', {
        params: { id: record.id, is_public: isPublic, type: isFolder ? 'folder' : 'file' }
      })
        .then(res => {
          setProperties(res);
        })
        ...
        <Descriptions column={1} size="small" bordered style={{ marginTop: 8 }}>
          ...
          <Descriptions.Item label={isFolder ? '文件夹名' : '文件名'}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {isFolder
                ? <span style={{ marginRight: 8, display: 'inline-flex', alignItems: 'center', lineHeight: 1 }}><FolderIcon size={18} open /></span>
            ...
            <Descriptions.Item label="包含内容">
              {properties ? (
                <span style={{ fontSize: 13 }}>
                  {properties.sub_folder_count} 个文件夹，{properties.file_count} 个文件
                  {properties.total_size > 0 && (
                    <span style={{ color: '#999', marginLeft: 8 }}>
                      (共 {formatFileSize(properties.total_size)})
                    </span>
                  )}
                </span>
              ) : !loading ? (
          ...
          <Descriptions.Item label="创建者">
            {properties?.created_by || record.created_by || '未知'}
          </Descriptions.Item>

          {/* 创建时间 */}
          <Descriptions.Item label="创建时间">
            <span style={{ fontSize: 12 }}>
              {formatDate(properties?.created_at || record.created_at)}
            </span>
          </Descriptions.Item>
          ...
          <Descriptions.Item label="修改时间">
            <span style={{ fontSize: 12 }}>
              {formatDate(properties?.updated_at || record.updated_at)}
            </span>
          </Descriptions.Item>
        </Descriptions>
      </Spin>
